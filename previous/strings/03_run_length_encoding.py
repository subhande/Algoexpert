...
def runLengthEncoding(string):
    def getCode(count, letter):
        if count < 10:
            return str(count) + letter
        code = ""
        rem = count % 9 
        times = count // 9
        return f"9{letter}" * times + f"{rem}{letter}"
    if len(string) == 1:
        return "1" + string
    ...
    for i in range(1, len(string)):
        if string[i] != current_letter:
            return getCode(count, current_letter) + runLengthEncoding(string[i:])
        count += 1
    return getCode(count, current_letter)
