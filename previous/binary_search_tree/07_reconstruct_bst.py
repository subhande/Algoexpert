...
"""
The pre-order traversal of a BST is a traversal technique that starts at the tree's root node and visits nodes in the following order: current node, left child, right child. You are given a non-empty array of integers representing the pre-order traversal of a BST. Write a function that creates the relevant BST and returns its root node. The input array will contain the values of BST nodes in the order in which these nodes would be visited with a pre-order traversal.
