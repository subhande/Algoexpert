...
def canTravelAroundCircle(distances, fuel, mpg, startIdx):
    numCities = len(distances)
    milesRemaining = 0

    for cityIdx in range(startIdx, startIdx + numCities):
        if milesRemaining < 0:
            return False
        currentCityIdx = cityIdx % numCities
...
def validStartingCity(distances, fuel, mpg):
    ...
    for startCityIdx in range(numCities):
        if canTravelAroundCircle(distances, fuel, mpg, startCityIdx):
            return startCityIdx
    return -1
...
def validStartingCity(distances, fuel, mpg):
    ...
    for cityIdx in range(1, numCities):
        distanceFromPreviousCity = distances[cityIdx - 1]
        fuelFromPreviousCity = fuel[cityIdx - 1]
        milesRemaining += fuelFromPreviousCity * mpg - distanceFromPreviousCity

        # Choose the starting city candidate greedily
        # by tracking the number of miles remaining
        # Select the candidate that has least number of miles remaining (mostly negative)
