...
def taskAssignment(k, tasks):
    ...
    for idx, task in enumerate(tasks):
        if task not in taskDurationsToIndices:
            taskDurationsToIndices[task] = []
        taskDurationsToIndices[task].append(idx)
    ...
    while start < end:
        task1Duration = sortedTasks[start]
        task2Duration = sortedTasks[end]
        pairedTasks.append([taskDurationsToIndices[task1Duration].pop(), taskDurationsToIndices[task2Duration].pop()])
        start += 1
        end -= 1
